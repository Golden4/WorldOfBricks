...
public class PanelsController : MonoBehaviour
{
    ...
    public void ShowTryBallsPanel(bool show)
    {
        ...
        else
        {
            ...
            for (int i = 0; i < ItemsInfo.Get.playersData.Length; i++)
            {
                ...
                if (ItemsInfo.Get.playersData[i].buyType != ItemsInfo.BuyType.Video && !User.GetInfo.userData[i].bought)
                {
                    ballsIndex.Add(i);
                }
            }
            ...
            if (ballsIndex.Count > 0)
            {
                for (int i = 0; i < ballsIndex.Count; i++)
                {
                    ...
                    ball.transform.Find("Icon").GetComponent<Image>().sprite = ItemsInfo.Get.playersData[ballsIndex[i]].ballSprite;
                    ball.transform.Find("Text").GetComponent<Text>().text = LocalizationManager.GetLocalizedText(ItemsInfo.Get.playersData[ballsIndex[i]].name);
    ...
    public void ShowBuyBallPanel(bool show)
    {
        ...
        else
        {
            ...
            ballItem.transform.Find("Icon").GetComponent<Image>().sprite = ItemsInfo.Get.playersData[ballTryingIndex].ballSprite;
            ballItem.transform.Find("Text").GetComponent<Text>().text = LocalizationManager.GetLocalizedText(ItemsInfo.Get.playersData[ballTryingIndex].name);

            Button[] btn = ballItem.GetComponentsInChildren<Button>();

            btn[0].GetComponentInChildren<Text>().text = ItemsInfo.Get.playersData[ballTryingIndex].price;
            btn[0].onClick.RemoveAllListeners();

            int coinAmount = int.Parse(ItemsInfo.Get.playersData[ballTryingIndex].price);
            ...
            string price = PurchaseManager.Ins.GetLocalizedPrice(ItemsInfo.Get.playersData[ballTryingIndex].purchaseID);
